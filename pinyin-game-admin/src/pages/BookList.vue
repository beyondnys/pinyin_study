<template>
  <div>
    <el-button type="primary" @click="openDialog()">新增练习册</el-button>
    <el-table :data="list" v-loading="loading" style="margin-top: 16px">
      ...
      <el-table-column label="操作" width="220">
        <template #default="{ row }">
          <el-button link type="primary" @click="$router.push(`/questions/${row.id}`)">题目</el-button>
...
import { createBook, deleteBook, fetchBooks, updateBook } from '@/api/books'
...
async function save() {
  if (form.id) await updateBook(form.id, { title: form.title, description: form.description, status: form.status })
  else await createBook({ title: form.title, description: form.description, status: form.status })
  visible.value = false
